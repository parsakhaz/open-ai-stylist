    ...
    <div className="min-h-full bg-gradient-to-br from-amber-50 to-yellow-50"
      ...
      <div className="container mx-auto p-6 md:p-12 pt-8">
        ...
        {activeTab === 'try-ons' ? (
          ...
          <>
            {(() => {
              // Split moodboards into manual and auto-generated
              const manualMoodboards = moodboards.filter(board => !board.isAutoGenerated);
              const autoGeneratedMoodboards = moodboards.filter(board => board.isAutoGenerated);
              
              return (
                <div className="space-y-12">
                  {/* Manual Moodboards Section */}
                  {manualMoodboards.length > 0 && (
                    <div>
                      <div className="flex items-center gap-3 mb-6">
                        <h2 className="text-xl font-semibold text-gray-900">Your Moodboards</h2>
                        <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded-full text-sm">
                          {manualMoodboards.length}
                        </span>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {manualMoodboards.map((board, boardIndex) => (
                  ...
                  {/* Auto-Generated Moodboards Section */}
                  {autoGeneratedMoodboards.length > 0 && (
                    <div>
                      <div className="flex items-center gap-3 mb-6">
                        <h2 className="text-xl font-semibold text-gray-900">Auto-Generated Inspiration</h2>
                        <span className="bg-purple-100 text-purple-600 px-2 py-1 rounded-full text-sm">
                          {autoGeneratedMoodboards.length}
                        </span>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {autoGeneratedMoodboards.map((board, boardIndex) => (
                          <div key={board.id} className="relative">
                            <MoodboardCard board={board} index={boardIndex} />
                  ...
                  {/* Empty state for no manual moodboards */}
                  {manualMoodboards.length === 0 && autoGeneratedMoodboards.length > 0 && (
                    <div className="text-center py-12">
                      <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-8 border border-gray-200 shadow-sm">
                        <Palette className="w-12 h-12 mx-auto text-gray-400 mb-4" />
                        <h3 className="text-lg font-semibold mb-2 text-gray-900">Create Your First Moodboard</h3>
                        <p className="text-gray-600 mb-4">
                          Browse products in chat and create personalized moodboards
                        </p>
                        <Link href="/chat">
                          <Button className="bg-black hover:bg-gray-800 text-white px-6 py-2 rounded-lg">
                            Start Styling <ArrowRight className="ml-2 h-4 w-4" />
